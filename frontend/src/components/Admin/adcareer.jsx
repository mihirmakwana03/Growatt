...
const CareerSection = () => {
  ...
  const handleInsertCareer = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`${API_URL}/${editId}`, formState);
        setCareers((prev) =>
          prev.map((career) =>
            career._id === editId ? { ...career, ...formState } : career
          )
        );
        setIsEditing(false);
        setEditId(null);
  ...
  const handleEditCareer = (career) => {
    setFormState({
      ...
      jobRequire: career.jobRequire,
      jobResponse: career.jobResponse,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center text-black">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl">
            <h3 className="text-xl font-bold mb-4 text-center">
              {isEditing ? "Edit Career" : "Add Career"}
            </h3>
